...
export async function POST(req: Request) {
  try {
    ...
    // ─── 사주 오행 계산 (child 또는 birthDate가 입력된 경우 모두 반영) ───
    const sajuText = birthDate
      ? (() => {
...
- 출신 국가가 명시된 경우, 이름 철자만으로 어원을 추측하지 말고 해당 국가의 언어·문화권을 최우선 기준으로 어원·의미를 분석할 것 (예: 같은 "Maria"라도 출신 국가가 필리핀/스페인/러시아인지에 따라 문화적 맥락이 다름)

[성별 작명 원칙 — 필수 준수]
- 남성(남자/male): 강하고 듬직한 느낌의 한자·음운 우선
  한자: 강할 강(强), 클 대(大), 빛날 환(煥), 굳셀 건(健), 씩씩할 웅(雄) 계열
  음운: 받침이 있는 묵직한 음절 선호 (현·준·민·혁·진·우 등)
- 여성(여자/female): 우아하고 섬세한 느낌의 한자·음운 우선
  한자: 아름다울 미(美), 빛날 영(英), 봄 춘(春), 맑을 청(淸), 부드러울 유(柔) 계열
  음운: 모음이 풍부하고 부드러운 음절 선호 (연·윤·아·서·혜·나 등)
- 중성(neutral): 성별 구분 없이 자연스러운 현대 한국 이름 — 밝을 하(夏), 온화할 온(溫) 등 중성적 계열
반드시 지정된 성별에 맞는 한자·음운을 사용할 것. 아래 "이름 스타일" 지시가 성별과 다르면(예: 남성이지만 여성적인 이름 희망) 스타일 지시를 우선 반영하되, caution 필드에 그 사실을 명시할 것.

[사주(생년월일) 기반 오행 보완 — birthDate가 주어진 경우 필수 반영]
입력 조건에 사주팔자 분석이 포함되어 있다면, 부족한 오행을 보완하거나 강한 오행과 상생하는 한자를 우선 선택하고, story 필드에 사주 오행을 어떻게 반영했는지 반드시 설명할 것.
...
- 성씨: ${familyName || "없음"}
- 국적/출신 국가: ${originCountry || "미지정"} ← 명시된 경우 이 국가의 언어·문화권을 기준으로 어원·의미를 분석할 것
- 성별: ${gender || "미지정"} ← 반드시 이 성별에 맞는 한자·음운으로 설계할 것
- 변환 방향/방법: ${purpose || "없음"} (발음대로 / 의미 기반 / 혼합)
- 이름 스타일·분위기 (남성적/여성적/혼용 및 성격): ${styleKeywords || "없음"}
...
[
  {
    ...
    "hanja_meaning": "한자 각 글자의 뜻을 구체적으로 (예: 旻=가을하늘 민 — 넓고 높은 가을 하늘, 俊=준걸 준 — 재능이 빼어난 사람)",
    ...
    "japanese_kana": "가나 표기",
    "japanese_reading": "로마자 읽기",
    "meaning": "이름 전체의 뜻을 구체적으로 (3-4문장, 필수): (1)각 한자/음절이 담은 뜻 (2)그 뜻들이 합쳐졌을 때의 전체 의미 (3)원본 외국 이름의 어원·의미와 정확히 어떻게 연결되는지 — '좋은 이름입니다' 같은 추상적 서술 금지, 반드시 구체적 근거 제시",
    "story": "원본 이름 어원 분석 → 한국 이름 설계 과정 상세 설명 (3-4문장): 원본 언어권 문화·어원 → 한자 선택 이유 → 오행·획수·성씨 조화 → (생년월일 입력 시) 사주 오행 보완 반영",
